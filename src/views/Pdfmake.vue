  ...
  <div class="demo-page">
    ...
    <div class="content-grid">
      ...
      <main class="demo-panel">
        <div class="controls">
          <button @click="generateSimplePDF" class="btn btn-primary">📄 生成简单PDF</button>
          <button @click="generateTablePDF" class="btn btn-secondary">📊 生成表格PDF</button>
          <button @click="generateComplexPDF" class="btn btn-info">📑 生成复杂文档</button>
          <button @click="generateReportPDF" class="btn btn-warning">📈 生成完整报告</button>
        </div>
...
const generateSimplePDF = () => {
  const docDefinition: any = {
    content: [
      { text: 'pdfmake 示例文档', style: 'header' },
      { text: '声明式 PDF 生成', style: 'subheader' },
      {
        text: '这是一个使用 pdfmake 生成的简单 PDF 文档。pdfmake 提供了非常友好的声明式 API，可以轻松创建结构化的 PDF 文档。',
        margin: [0, 10, 0, 10],
      },
  ...
  pdfMake.createPdf(docDefinition).download('pdfmake-simple.pdf')
}
...
const generateComplexPDF = () => {
  const docDefinition: any = {
    ...
    content: [
      ...
      { text: '前端打印方案选型', style: 'subtitle' },
      { text: `生成日期：${new Date().toLocaleDateString()}`, style: 'date' },
  ...
  pdfMake.createPdf(docDefinition).download('pdfmake-complex.pdf')
}
...
const generateReportPDF = () => {
  ...
  pdfMake.createPdf(docDefinition).download('完整调研报告.pdf')
}
</script>
